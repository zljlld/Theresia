...
def apply_speaker_compare_enabled(enabled: bool):
    try:
        final_enabled = APP.service.set_enable_speaker_compare(bool(enabled))
    except Exception as exc:
        final_enabled = APP.service.get_enable_speaker_compare()
        APP.append_log(f"[\u7f51\u9875] \u58f0\u7eb9\u5bf9\u6bd4\u5f00\u5173\u66f4\u65b0\u5931\u8d25\uff1a{exc}")
        status, speaker, text, emotion, reg_state, logs = APP.ui_snapshot()
        return final_enabled, status, speaker, text, emotion, reg_state, logs

    APP.append_log(f"[\u7f51\u9875] \u58f0\u7eb9\u5bf9\u6bd4\u5df2{'\u542f\u7528' if final_enabled else '\u7981\u7528'}")
    status, speaker, text, emotion, reg_state, logs = APP.ui_snapshot()
    return final_enabled, status, speaker, text, emotion, reg_state, logs
...
with gr.Blocks(title="ASR 声纹管理网页", css=custom_css) as demo:
        ...
        with gr.Tabs():
            with gr.Tab("阈值设置"):
                with gr.Group(elem_classes=["x-card"]):
                    ...
                    speaker_compare_enabled = gr.Checkbox(
                        label="\u542f\u7528\u58f0\u7eb9\u5bf9\u6bd4",
                        value=APP.service.get_enable_speaker_compare(),
                    )
    ...
    speaker_compare_enabled.change(
        fn=apply_speaker_compare_enabled,
        inputs=[speaker_compare_enabled],
        outputs=[speaker_compare_enabled, status_box, speaker_box, text_box, emotion_box, reg_state, log_box],
    )
    start_reg_btn.click(
